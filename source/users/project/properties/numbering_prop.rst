.. _project/properties/numbering_prop:
...
Every time that a new folio is added to the `project`_ or an `element`_ or `conductor`_ is drawn in the `workspace`_; the `folio`_, `element`_ or `conductor`_ is codified according to the selected pattern in `auto numbering selection panel`_.
